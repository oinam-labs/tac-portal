...
export const ManifestPrintView = React.forwardRef<HTMLDivElement, Props>(({ manifest, items }, ref) => {
    return (
        <div ref={ref} className="p-8 bg-white text-black font-sans print-container">
            ...
            <div className="border-b-2 border-black pb-4 mb-6">
                <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                        ...
                        <div className="text-sm mt-1">Date: {new Date(manifest.created_at).toLocaleDateString()}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8 text-sm">
                <div className="border p-4">
                    <div className="font-bold text-xs uppercase text-gray-500 mb-2">Sector</div>
                    <div className="flex justify-between items-center text-lg font-bold">
                        <span>{manifest.from_hub?.code} ({manifest.from_hub?.name})</span>
                        <span>→</span>
                        <span>{manifest.to_hub?.code} ({manifest.to_hub?.name})</span>
                    </div>
                </div>
                <div className="border p-4">
                    <div className="font-bold text-xs uppercase text-gray-500 mb-2">Transport Details</div>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <div className="text-xs text-gray-500">Vehicle / Flight</div>
                ...
                <tbody>
                    {items.map((item, index) => (
                        <tr key={item.id} className="border-b border-gray-300">
                            ...
                            <td className="p-2">{item.shipment.destination_hub_id === manifest.to_hub_id ? 'Local' : 'Transit'}</td>
                            <td className="p-2 text-right">{item.shipment.package_count}</td>
                            <td className="p-2 text-right">{item.shipment.total_weight}</td>
                            <td className="p-2 pl-4 italic text-gray-500">{item.shipment.special_instructions || '-'}</td>
                        </tr>
                    ))}
                </tbody>
                <tfoot>
