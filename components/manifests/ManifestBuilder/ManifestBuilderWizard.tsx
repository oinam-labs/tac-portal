...
function combineDateTimeWithPeriod(date: Date, hour?: string, minute?: string, period?: 'AM' | 'PM'): string {
    ...
    const handleStep1Submit = async () => {
        ...
        if (!builder.manifest) {
            const params: CreateManifestParams = {
                ...
                etd: setupData.flightDate && setupData.etdHour
                    ? combineDateTimeWithPeriod(setupData.flightDate, setupData.etdHour, setupData.etdMinute, setupData.etdPeriod)
                    : undefined,
                eta: setupData.flightDate && setupData.etaHour
                    ? combineDateTimeWithPeriod(setupData.flightDate, setupData.etaHour, setupData.etaMinute, setupData.etaPeriod)
                    : undefined,
                // TRUCK
                ...
                dispatchAt: setupData.dispatchDate
                    ? combineDateTimeWithPeriod(setupData.dispatchDate, setupData.dispatchHour, setupData.dispatchMinute, setupData.dispatchPeriod)
                    : undefined,
    ...
    return (
        <>
            <Dialog open={open} onOpenChange={handleCancel}>
                <DialogContent
                    className="w-[100vw] h-[100dvh] md:w-[min(1200px,92vw)] md:h-[min(88vh,900px)] md:max-h-[90vh] lg:w-[min(1320px,88vw)] md:rounded-lg rounded-none flex flex-col gap-0 p-0 max-w-none overflow-hidden"
                >
                    {/* Header */}
                                    ...
                                    <TooltipProvider>
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <span tabIndex={0}> {/* Span needed for disabled button tooltip */}
                                                    <Button
                                                        onClick={handleNext}
                                                        disabled={
                                            ...
                                            <TooltipContent>
                                                {currentStep === 1 && !isStep1Valid && <p>Please fill all required fields correctly</p>}
                                                {currentStep === 2 && builder.items.length === 0 && <p>Add at least one shipment to proceed</p>}
                                            </TooltipContent>
                                        </Tooltip>
                                    </TooltipProvider>
                                ) : (
            ...
            <AlertDialog open={showCloseConfirm} onOpenChange={setShowCloseConfirm}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action will close the manifest immediately. Once closed, no more
                            shipments can be added and the manifest will be ready for dispatch.
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            Any shipments added to the manifest will be preserved.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
