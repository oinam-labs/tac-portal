...
const resultVariant: Record<'success' | 'duplicate' | 'error', "default" | "secondary" | "destructive" | "outline"> = {
    success: "default",
    duplicate: "outline",
    error: "destructive",
}
    ...
    return (
        <div className={cn("rounded-xl border bg-background", className)}>
            ...
            <ScrollArea className="h-40">
                {entries.length === 0 ? (
                    <div className="p-4 text-xs text-muted-foreground">No scans yet. Start scanning AWBs.</div>
                ) : (
                    <div className="divide-y">
                        {entries.map((entry, index) => (
                            <div key={`${entry.manifest_item_id ?? entry.shipment_id ?? index}-${entry.timestamp}`} className="px-4 py-2 text-xs">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Badge
                                        ...
                                        className="text-[10px]"
                                    >
                                        {entry.duplicate ? "DUPLICATE" : entry.success ? "SUCCESS" : entry.error ?? "ERROR"}
                                    </Badge>
                                </div>
