...
import { Camera, CheckCircle2, Keyboard, Scan, XCircle, AlertCircle, RotateCcw, Package } from "lucide-react"
...
export function StepAddShipments({
    ...
    const totalPieces = items.reduce((sum, item) => sum + (item.shipment?.package_count || item.shipment?.total_packages || 0), 0)
            ...
            <div className="lg:col-span-2 space-y-4 flex flex-col h-full overflow-hidden">
                <Card className="border-border bg-card/50 shrink-0">
                    ...
                    <CardContent className="space-y-4">
                        <Tabs value={scanMode} onValueChange={(value) => setScanMode(value as ManifestScanMode)} className="w-full">
                            ...
                            <TabsContent value="scanner" className="mt-4">
                                ...
                                <form onSubmit={scanner.handleSubmit} className="mt-2 opacity-0 h-0 overflow-hidden">
                                    <Input
                                        ref={inputRef}
                                        value={scanner.inputValue}
                                        onChange={scanner.handleChange}
                ...
                {scanner.lastResult && (
                    <Card
                        className={cn(
                            ...
                            scanner.lastResult.success && !scanner.lastResult.duplicate && 'border-green-500/50 bg-green-500/10',
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="space-y-1 flex-1">
                                    <p
                                        className={cn(
                                            'text-sm font-medium',
                                            scanner.lastResult.success && !scanner.lastResult.duplicate && 'text-green-700 dark:text-green-400',
                ...
                <div className="grid grid-cols-4 gap-2 shrink-0">
                    ...
                    <Card className="border-border bg-card/50">
                        <CardContent className="p-3 text-center">
                            <p className="text-lg font-bold truncate" title={totalWeight.toFixed(1)}>{totalWeight.toFixed(1)}</p>
                    ...
                    <CardHeader className="pb-4 shrink-0">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    {items.length} shipment{items.length !== 1 ? 's' : ''} added • {totalPieces} pieces • {totalWeight.toFixed(1)} kg
                                </CardDescription>
                            </div>
                        </div>
                    </CardHeader>
