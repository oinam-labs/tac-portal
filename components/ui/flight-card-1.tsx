                    ...
                    <div className="relative h-72 w-full overflow-hidden">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 h-32 px-8 flex items-end justify-between pb-8">
                            <div className="text-left z-10">
                                <div className="text-xs text-muted-foreground font-mono mb-1">{departureTime} - DEPART</div>
                                <div className="text-6xl font-bold text-foreground tracking-tighter drop-shadow-lg">{departureCode}</div>
                                <div className="text-sm font-medium text-primary uppercase tracking-widest">{departureCity}</div>
                            </div>
                            ...
                            <div className="text-right z-10">
                                <div className="text-xs text-muted-foreground font-mono mb-1">ARRIVE - {arrivalTime}</div>
                                <div className="text-6xl font-bold text-foreground tracking-tighter drop-shadow-lg">{arrivalCode}</div>
                                <div className="text-sm font-medium text-primary uppercase tracking-widest">{arrivalCity}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-8 pt-6 bg-card/60 relative">
                        ...
                        <div className="mt-8 pt-6 border-t border-dashed border-border flex justify-between items-center opacity-70 group-hover:opacity-100 transition-opacity">
                            ...
                            <div className="flex gap-1.5">
                                {[1, 2, 3, 4].map(i => <div key={i} className="h-1 w-8 bg-foreground/10 rounded-full" />)}
                            </div>
                        </div>
                    </div>
                </div>
...
const InfoBlock = ({ label, value, sub, highlight }: { label: string; value: string; sub: string; highlight?: boolean }) => (
    <div className="flex flex-col items-center group/item hover:bg-foreground/5 p-2 rounded transition-colors cursor-default">
        <span className="text-[9px] uppercase tracking-[0.2em] text-muted-foreground mb-2 opacity-70">{label}</span>
        <span className={cn("text-lg font-bold font-mono mb-1", highlight ? "text-primary" : "text-foreground")}>{value}</span>
