      ...
      <TooltipPrimitive.Root>
        <TooltipPrimitive.Trigger asChild>
          <Button
            ...
            onClick={onClick}
            disabled={disabled}
            className={cn('h-8 w-8 p-0 hover:bg-muted', isActive && 'bg-muted text-primary')}
          >
            {children}
          </Button>
...
const ToolbarDivider = memo(() => <div className="mx-1 h-6 w-px bg-border" />);
...
export const RichTextEditor = forwardRef<RichTextEditorRef, RichTextEditorProps>(
  (
    ...
    if (!editor) {
      return (
        <div
          className={cn('rounded-md border border-input bg-background animate-pulse', className)}
          style={{ minHeight }}
        />
      );
    }
