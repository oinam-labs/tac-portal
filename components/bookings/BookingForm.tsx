...
export const BookingForm: React.FC<BookingFormProps> = ({ onSuccess, onCancel }) => {
    ...
    const {
        ...
        defaultValues: {
            ...
            consignor: { name: '', phone: '', address: '', city: 'Imphal', state: 'Manipur', zip: '795001' },
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <Card>
                ...
                <CardContent>
                    <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">We will contact you on this number for shipment updates.</p>
                    </div>
                </CardContent>
            </Card>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        <div className="space-y-1">
                            ...
                            {errors.consignor?.name && <p className="text-xs text-red-500">{errors.consignor.name.message}</p>}
                        </div>
                        <div className="space-y-1">
                            <Label>Phone</Label>
                            <Input {...register('consignor.phone')} placeholder="Sender Phone" />
                            {errors.consignor?.phone && <p className="text-xs text-red-500">{errors.consignor.phone.message}</p>}
                        </div>
                        <div className="space-y-1">
                            <Label>Address</Label>
                            <Input {...register('consignor.address')} placeholder="Address Line 1" />
                            {errors.consignor?.address && <p className="text-xs text-red-500">{errors.consignor.address.message}</p>}
                        </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        <div className="space-y-1">
                            ...
                            {errors.consignee?.name && <p className="text-xs text-red-500">{errors.consignee.name.message}</p>}
                        </div>
                        <div className="space-y-1">
                            <Label>Phone</Label>
                            <Input {...register('consignee.phone')} placeholder="Receiver Phone" />
                            {errors.consignee?.phone && <p className="text-xs text-red-500">{errors.consignee.phone.message}</p>}
                        </div>
                        <div className="space-y-1">
                            <Label>Address</Label>
                            <Input {...register('consignee.address')} placeholder="Address Line 1" />
                            {errors.consignee?.address && <p className="text-xs text-red-500">{errors.consignee.address.message}</p>}
                        </div>
                        <div className="grid grid-cols-3 gap-3">
                            <div className="space-y-1">
                                <Label>City</Label>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => append({ length: 0, width: 0, height: 0, weight: 0, count: 1 })}>
                ...
                <CardContent className="space-y-4">
                    ...
                    {errors.volumeMatrix && <p className="text-xs text-red-500">{errors.volumeMatrix.message}</p>}
                </CardContent>
            </Card>
