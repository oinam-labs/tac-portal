  ...
  return (
    ...
    >
      ...
      <div ref={ringsGroupRef} className="relative w-full h-full transform-style-3d">
        ...
        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-[100%] rounded-full border-[1px] border-primary/20 transform-gpu rotate-x-90" />{' '}
        {/* Equator */}
        {[...Array(3)].map((_, i) => (
          <div
            key={`lat-north-${i}`}
            className="absolute inset-x-0 top-1/2 -translate-y-1/2 rounded-full border-[1px] border-foreground/5"
            style={{
              height: `${85 - i * 25}%`,
