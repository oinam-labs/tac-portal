...
export function HeroOverlays() {
    ...
    return (
        <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden text-primary/80 font-mono text-[10px] md:text-xs select-none">
            ...
            >
                <Badge variant="outline" className="bg-background/20 backdrop-blur-md border-primary/20 text-primary gap-2">
                    <div className="w-2 h-2 rounded-full bg-status-live animate-pulse" />
                    SYSTEM_OPTIMAL
                </Badge>
                <div className="flex items-center gap-4 text-muted-foreground/60">
                    <div className="flex items-center gap-1"><Wifi className="w-3 h-3" /> 100%</div>
                    <div className="flex items-center gap-1"><Radio className="w-3 h-3" /> 5G_TAC</div>
                    <div className="flex items-center gap-1"><Activity className="w-3 h-3" /> LOW_LATENCY</div>
                </div>
            </motion.div>
