...
function useMockChat({ onFinish }: { onFinish?: (opts: { message: Message }) => void } = {}) {
    const [messages, setMessages] = useState<Message[]>([
        {
            id: '1',
            role: 'assistant',
            parts: [{ type: 'text', text: 'Khurumjari! I am TAC-Bot. How can I help you with your logistics today?' }],
        },
    ]);
    ...
    const sendMessage = async ({ parts }: { parts: { type: 'text'; text: string }[] }) => {
        ...
        setTimeout(async () => {
            ...
            let responseText = "Thagatchari for your message! I am a simulated UI demo. In the real app, I would connect to the TAC Cargo AI backend to track shipments (e.g., AWB-12345) or answer service queries. How else can I assist?";

            if (userText.toLowerCase().includes('track') || userText.toLowerCase().includes('awb')) {
                responseText = "I can help with tracking! Please provide your **AWB Number**, **Full Name**, and **Phone Number** so I can check the status.";
            } else if (userText.toLowerCase().includes('price') || userText.toLowerCase().includes('rate')) {
                responseText = "For accurate pricing, I need the **weight**, **dimensions**, and **destination** of your shipment. We offer Air Cargo and Surface Transport.";
            }
            ...
            for (let i = 0; i < words.length; i++) {
                ...
                setMessages(prev => prev.map(m => m.id === botMsgId ? { ...m, parts: [{ type: 'text', text: currentText }] } : m));
            }

            setStatus('ready');
            if (onFinish) onFinish({ message: { ...botMsg, parts: [{ type: 'text', text: currentText }] } });
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[450px] w-[95vw] h-[600px] max-h-[85vh] p-0 gap-0 border-border/50 bg-background/95 backdrop-blur-xl shadow-2xl flex flex-col overflow-hidden data-[state=closed]:slide-out-to-bottom-[48%] data-[state=open]:slide-in-from-bottom-[48%] duration-500 rounded-3xl">
                ...
                {/* Messages Area */}
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth bg-gradient-to-b from-background to-muted/10">
                    {messages.length > 0 ? (
                        messages.map((m) => (
                            <div key={m.id} className="flex flex-col gap-1 animating-in slide-in-from-bottom-2 duration-300 fade-in">
                                <div className={cn(
