...
export const generateDashboardReport = (data: ReportData) => {
    ...
    // --- KPI Summary ---
    const active = data.shipments.filter(s => ['RECEIVED_AT_ORIGIN', 'IN_TRANSIT', 'RECEIVED_AT_DEST'].includes(s.status)).length;
    ...
    const pendingRevenue = data.invoices.filter(i => ['ISSUED', 'OVERDUE'].includes(i.status)).reduce((acc, inv) => acc + inv.total, 0);
    ...
    const tableRows = data.shipments.slice(0, 15).map(s => [
        s.awb_number,
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Page ${i} of ${pageCount} - TAC Logistics Confidential`, 105, 290, { align: 'center' });
    }
