...
async function generate1DBarcode(text: string): Promise<string> {
  ...
  try {
    ...
    return await generateBarcodeDataURL(sanitized, 'pdf');
  } catch (e) {
...
export async function generateShipmentLabel(shipment: Shipment): Promise<string> {
  ...
  page.drawText((shipment as any).gstNumber || '07AAMFT6165B1Z3', {
    x: 16 + colW,
    y: y - 22,
    size: 8,
    font: fontBold,
...
export async function generateLabelPDF(data: LabelData): Promise<string> {
  ...
  if (!data?.awb) throw new Error('Invalid label data: missing AWB');
...
export async function generateEnterpriseInvoice(invoice: Invoice): Promise<string> {
  ...
  page.drawText('TAPAN ASSOCIATE CARGO', {
    x: margin + 34,
    y: logoY - 18,
    size: 6,
    font: fontBold,
  ...
  page.drawText('Acct: Tapan Associate Cargo', {
    x: margin,
    y: footerY - 24,
    size: 8,
    font: fontRegular,
