...
async def run_test():
    ...
    try:
        ...
        # -> Try to input password using keyboard send_keys action or clear and re-input email and password fields.
        frame = context.pages[-1]
        ...
        # -> Re-input email and password carefully, then click Sign In again to attempt login.
        frame = context.pages[-1]
        # Clear email field before re-input.
        elem = frame.locator('xpath=html/body/div/div/div/main/div/div[3]/form/div/input').nth(0)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Full CRUD operations successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: ADMIN role could not perform full CRUD operations and management of staff roles, hubs, and organization entities ensuring data consistency and RBAC enforcement.")
